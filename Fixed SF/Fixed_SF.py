...
NumDev=50; #no. of mobile devices
PacketLenBytes=40;
...
location_node=[]
######################################################################
def node_distribution_real1():
    print("yes")
    theta_m_real=.1
    location_node_real=[]
    a=[]
    color_index=[]
    def node_distribution_real(r,st,col):
        #print("colour",st)
        location=int(np.ceil(np.random.randint(0,359)/theta_m_real))
        X = r * math.cos(location*.1)  
        Y = r * math.sin(location*.1)  
        location_node_real.append((X,Y))
        color_index.append(col)
        color_SF=['Fixed SF']
        colors_1=['red','blue','yellow','brown','orange','green']
        a.append(st)
        #print(colors_1[0])
        if len(location_node_real)==1200:
            #print("colour_index",color_index,r,len(color_index))
            xs=[x[0] for x in location_node_real]
           
            ys=[x[1] for x in location_node_real]
            
            plt.scatter(xs,ys, label='Fixed SF')
                #plt.scatter(xs[mask,0],ys[mask,0],c=colors[i],label=color_SF[i])
            #plt.scatter(xs,ys,c=a)
           
           
            plt.xlabel('Fixed SF allocation Scheme')
            plt.legend()
            plt.show()
    for r in range(1,7):
        if r==1:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(0,1),'red',0)
        if r==2:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(1,2),'blue',1)
        if r==3:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(2,3),'yellow',2)
        if r==4:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(3,4),'brown',3)
        if r==5:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(4,5),'orange',4)
        if r==6:
            for i in range(200):
                location=node_distribution_real(np.random.uniform(5,6),'green',5)
############################################################
...
class Node():
    ...
    def __init__(self,space="ring",mobility="SRW",num=1):#for symmetric random walk
        ...
        self.p=float(load_context(strn,np.random.uniform(.9,.999))); #probability of transmitting in a sample duration
...
real_distribution=node_distribution_real1()
print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
#generate the nodes
...
for j in nodes:
    ...
    over_all_attempt=j.total_attempt+over_all_attempt
